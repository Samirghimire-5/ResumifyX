...
const Templates = () => {
  ...
  const { data: templates, isLoading, isError } = useQuery({
    queryKey: ['templates'],
    queryFn: async () => {
      const { data } = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/api/templates`)
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="bg-red-50 border-l-4 border-red-500 p-4 rounded-md shadow">
          <div className="flex items-center">
            ...
            <div className="ml-3">
              <p className="text-red-700 font-medium">Failed to load templates. Please try again later.</p>
            </div>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8">
        ...
        <p className="text-gray-600 mb-6">Browse our collection of professionally designed templates</p>
      ...
      {filteredTemplates && filteredTemplates.length > 0 ? (
            ...
            >
              ...
              <div className="p-5">
                <div className="flex items-start justify-between">
                  <h2 className="text-xl font-semibold text-gray-800 truncate">{template.name}</h2>
                  {template.category && (
                ...
                <p className="text-gray-600 mt-2 text-sm line-clamp-2 min-h-12">{template.description}</p>
                
                <div className="mt-4 pt-4 border-t border-gray-100 flex justify-between items-center">
                  <div className="text-sm text-gray-500">
                    {template.downloads ? `${template.downloads} downloads` : "New"}
                  </div>
                  <button className="flex items-center bg-blue-600 hover:bg-blue-700 text-white px-3 py-1.5 rounded-lg text-sm font-medium transition-colors">
                    Use Template
        ...
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="text-gray-500 max-w-md">
            We couldn't find any templates matching your search. Try using different keywords or browse all templates.
          </p>
          {searchTerm && (
            <button 
